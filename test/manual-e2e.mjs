...
async function main() {
  ...
  try {
    ...
    record("recap-fits-the-card", recapText.length > 0 && recapText.length <= 600, `length=${recapText.length}`);
    // The opposite failure: a card so short it carries nothing worth reading.
    record("recap-carries-substance", recapText.length >= 60, `length=${recapText.length}`);
    ...
    record("recap-has-no-reasoning-leak", recapText.length > 0 && !recapText.toLowerCase().includes(thinkOpen), recapText.slice(0, 40) || "no text was persisted");
    record("recap-ends-on-a-finished-sentence", /[。！？；.!?;]$/.test(recapText), recapText.slice(-24));
  } finally {
